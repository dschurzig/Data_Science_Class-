...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        pos = len(filtered_df[(filtered_df['Launch Site']==entered_site) & (filtered_df['class']==1)])
        neg = len(filtered_df[(filtered_df['Launch Site']==entered_site) & (filtered_df['class']==0)])
        data = pd.DataFrame(np.array([neg,pos])/(neg+pos),
        index=['not successful (class=0)','successful (class=1)'],columns=['class'])
        fig = px.pie(data, values='class',
        names = data.index,
...
def scatter_update(entered_site,payload_range):
